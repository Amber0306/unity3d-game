...
static public class EventDelegateEditor
{
	...
	{
        UnityEngine.MonoBehaviour[] comps = target.GetComponents<UnityEngine.MonoBehaviour>();

		List<Entry> list = new List<Entry>();

		for (int i = 0, imax = comps.Length; i < imax; ++i)
		{
            UnityEngine.MonoBehaviour mb = comps[i];
			if (mb == null) continue;
	...
	static public bool Field (Object undoObject, EventDelegate del, bool removeButton, bool minimalistic)
	{
		...
		GUI.changed = false;
		bool retVal = false;
        UnityEngine.MonoBehaviour target = del.target;
		bool remove = false;
		...
		if (removeButton && (del.target != null || del.isValid))
		{
			...
			{
				target = EditorGUILayout.ObjectField("Notify", del.target, typeof(UnityEngine.MonoBehaviour), true) as UnityEngine.MonoBehaviour;
			}
		...
		else target = EditorGUILayout.ObjectField("Notify", del.target, typeof(UnityEngine.MonoBehaviour), true) as UnityEngine.MonoBehaviour;
		...
		if (del.target != null && del.target.gameObject != null)
		{
			...
			if (choice > 0 && choice != index)
			{
				...
				NGUIEditorTools.RegisterUndo("Delegate Selection", undoObject);
				del.target = entry.target as UnityEngine.MonoBehaviour;
