...
public class UIToggledComponents : UnityEngine.MonoBehaviour
{
	public List<UnityEngine.MonoBehaviour> activate;
	public List<UnityEngine.MonoBehaviour> deactivate;

	// Deprecated functionality
	[HideInInspector][SerializeField] UnityEngine.MonoBehaviour target;
	...
	public void Toggle ()
	{
		if (enabled)
		{
			...
			{
                UnityEngine.MonoBehaviour comp = activate[i];
				comp.enabled = UIToggle.current.value;
			}

			for (int i = 0; i < deactivate.Count; ++i)
			{
                UnityEngine.MonoBehaviour comp = deactivate[i];
				comp.enabled = !UIToggle.current.value;
			}
		}
	}
