...
public class MainCamerab : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        //transform.position = m_player.position + m_player.TransformDirection(Offset);
        transform.position = Vector3.Lerp(transform.position ,m_player.position + ctrl.TransformDirection(Offset), Time.deltaTime*smoothing);//321
        //摄像机朝向角色
        ...
        if (Physics.Linecast(m_player.position + Vector3.up, transform.position, out hit))
        {
            ...
            else
            {
                ...
                last.GetComponent<Renderer>().enabled = true;
            }
        }
    }
}
